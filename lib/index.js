...
actions_toolkit_1.Toolkit.run(async (tools) => {
    ...
    if (!skipAuthors.has(user.login)) {
        if (titlePrefixes.length) {
            assert_1.default.ok(titlePrefixes.some((prefix) => title.startsWith(prefix)), `Pull request title must begin with one of "${titlePrefixes.join('", "')}"`);
        }
        if (requireIssue) {
            assert_1.default.ok(/((refs?|close(d|s)?|fix(ed|es)?)(:)?\s*((\#\d+)|(https?:\/\/.+\/.+\/.+\/issues\/\d+)))|(#minor)|(#release)/i.test(body), [
